...
void cdf53(float *out, float *in, int N, int SO, int SI, int CH)
{
	...
	for (int i = 1; i < N-1; i += 2)
		for (int c = 0; c < CH; ++c)
			in[i*SI+c] -= (in[(i-1)*SI+c] + in[(i+1)*SI+c]) / 2.f;
	if (!(N&1))
		for (int c = 0; c < CH; ++c)
	...
	for (int i = 2; i < M; i += 2)
		for (int c = 0; c < CH; ++c)
			in[i*SI+c] += (in[(i-1)*SI+c] + in[(i+1)*SI+c]) / 4.f;
...
void icdf53(float *out, float *in, int N, int SO, int SI, int CH)
{
	...
	for (int i = 2; i < M; i += 2)
		for (int c = 0; c < CH; ++c)
			out[i*SO+c] -= (out[(i-1)*SO+c] + out[(i+1)*SO+c]) / 4.f;

	for (int i = 1; i < N-1; i += 2)
		for (int c = 0; c < CH; ++c)
			out[i*SO+c] += (out[(i-1)*SO+c] + out[(i+1)*SO+c]) / 2.f;
	if (!(N&1))
		for (int c = 0; c < CH; ++c)
